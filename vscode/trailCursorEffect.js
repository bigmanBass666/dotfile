...
const TrailLength = 10 // Recommended value is around 8
...
function createTrail(options) {
  ...
  let particlesColor = options?.color || '#fff'
  ...
  function calculatePosition() {
    ...
    for (const particleIndex in particles) {
      ...
      // 粒子更新频率是由 createTrail 函数中的 calculatePosition 函数控制的，该函数在每一帧中更新粒子的位置。calculatePosition 函数通过调整粒子的位置来实现拖尾效果，但具体的速度并没有直接设置。
      // 如果你想要调整粒子更新的频率，可以修改 calculatePosition 函数中的两个系数：0.42 和 0.35。这两个系数决定了粒子位置更新的速度。
      // 增大这两个系数会减慢粒子的移动速度，减小这两个系数会加快粒子的移动速度。
      // 例如，你可以将这两个系数增大到 0.5 来加快粒子速度：
      x += (nextParticlePos.x - particlePos.x) * 0.17
      y += (nextParticlePos.y - particlePos.y) * 0.18
